...
async function postSingleMessage(message) {
  ...
  try {
    if (message.media) {
      ...
      await bot.sendMessage(destinationChannel, text, { parse_mode: 'HTML' });
    }
  } catch (err) {
  ...
  client.addEventHandler(async (event) => {
    ...
    if (groupedId) {
      ...
      const timeout = setTimeout(async () => {
        ...
        const caption = captionMessage ? formatMessage(captionMessage.message, captionMessage.entities || []) : '';
